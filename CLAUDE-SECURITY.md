[GENERATED USING CLAUDE CODE, OPUS 4.8] AS OF COMMIT 9f883370e5bc91c357ae97b1b6cc84c6ba185516
...
export const load = async ({ params }) => { const { uuid } = params const projectResponse = await fetchProjectFromUnifiedUUID(uuid) ... return { project: data.records[0] } }
...
const { recordId, ... } = await request.json() ... patchProjectForShip(recordId, newLog, "accepted") // accept ... addToJustifications({ ... }) // sendToAirtable awards currency
...
const decodedToken = jwt.decode(tokenBody.id_token) as {...} const airtableRes = await doesAdminExist(decodedToken?.slack_id || "")
...
const accessToken = cookies.get('access_token_new'); if (!accessToken || accessToken === "") { return { relogin: true } }
...
🟢 LOW / Informational and the raw client.query calls in db.ts all use $1 placeholders. ✅
