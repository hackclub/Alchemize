...
<Dialog.Root bind:open>
	...
	>
		...
		<form
			...
			class="flex-1 flex flex-col min-h-0 bg-zinc-950"
		>
			{#if mode === "update"}
			...
			<div
				class="flex-1 grid grid-cols-1 lg:grid-cols-12 min-h-0 overflow-hidden"
			>
				{#if mode === "update"}
					<div
						class="lg:col-span-4 flex flex-col border-b lg:border-b-0 lg:border-r border-zinc-900 h-full min-h-0"
					>
						<div class="p-6 pb-3 shrink-0">
							<h3
								class="text-xs font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-2"
							>
						...
						<div
							class="flex-1 overflow-y-auto px-6 pb-6 space-y-3 custom-scrollbar wrap-break-word"
						>
							{#each [...log].reverse() as entry}
								{#each [...entry.message].reverse() as msg, i}
									{#if msg.reviewerName != "user"}
										<div
											class="group border border-zinc-800 bg-zinc-900/30 rounded-xl p-4 transition hover:bg-zinc-900/50 border-l-4 overflow-auto {i ===
											0
												? entry.status === 1
				...
				<div
					class="{mode === 'update'
						? 'lg:col-span-8'
						: 'lg:col-span-12 max-w-3xl mx-auto w-full'} h-full flex flex-col min-h-0"
				>
					<div class="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
						...
						<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
							...
							<div class="space-y-2 {mode === 'update' ? 'sm:col-span-2' : ''}">
								...
								>
									<option value="" disabled selected={!hackatime}>Choose</option
									>
			...
			<div
				class="shrink-0 flex items-center justify-between gap-4 p-4 border-t border-zinc-900 bg-zinc-950/80 backdrop-blur-sm"
			>
				<div>
					{#if mode === "update"}
						<Button
							type="button"
							variant="default"
							class="hover:bg-primary/70 hover:-translate-y-px text-xs font-bold uppercase tracking-wider px-6 h-10 shadow-lg"
							onclick={ship}
						>
							{#if showRotator}
								<div
